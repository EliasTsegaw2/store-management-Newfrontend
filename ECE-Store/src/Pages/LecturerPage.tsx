...
import LecturerRequestForm from '../Components/LecturerRequestForm'; // Placeholder, create if needed
...
const LecturerPage: React.FC = () => {
  ...
  useEffect(() => {
    if (tab !== queryTab) {
      navigate(`/lecturer?tab=${tab}`, { replace: true });
    }
  }, [tab]);
  ...
  useEffect(() => {
    ...
    const fetchInventory = async () => {
      ...
      try {
        ...
        const res = await import('axios').then(ax => ax.default.get('/api/inventory', {
          params: { search },
      ...
      <Box display="flex" minHeight="100vh" bgcolor="#f5f6fa">
        ...
        <Container maxWidth="xl" sx={{ flex: 1, py: 4 }}>
          ...
          <Paper elevation={1} sx={{ p: 2, minHeight: 500 }}>
            {tab === 'request' && <LecturerRequestForm />}
            {tab === 'inventory' && (
              <>
                <Box mb={2}>
